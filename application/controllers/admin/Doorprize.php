...
class Doorprize extends MY_Controller {
	...
	public function __construct()
	{
		...
		$this->order 		= ['registration_id', 'participant_name', 'participant_nip', 'participant_nik', 'participant_unit_bisnis', 'participant_department', 'is_taken_doorprize', 'taken_doorprize_datetime'];
		$this->search 		= ['registration_id', 'participant_name', 'participant_nip', 'participant_nik', 'participant_unit_bisnis', 'participant_department', 'is_taken_doorprize', 'taken_doorprize_datetime'];
	...
	public function view()
	{
		...
		foreach ($res as $row) {
			...
			$col[] = ($row->is_taken_doorprize == 0 ? 'Belum Ambil' : 'Sudah Ambil' );
			$col[] = ($row->taken_doorprize_datetime ? date('d/m/Y H:i:s', strtotime($row->taken_doorprize_datetime)) : '-' );
			$data[] = $col;
			$a++;
		}
		$output = array(
